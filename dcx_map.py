...
DUMP_0_FILE = 'dumpwatch0'
DUMP_1_FILE = 'dumpwatch1'
PORT_NAME = 'ttyUSB0'
...
def _modify_cached_value(control_item, new_val):
    #print('dcx_map:890: ', control_item )
...
def _get_local_cache():
    with open(DUMP_0_FILE, 'rb') as f:
        read0 = f.read()
    with open(DUMP_1_FILE, 'rb') as f:
        read1 = f.read()
        local_cache = (read0, read1)
    return local_cache
...
def _get_cached_value(control_item):
    ...
    l_val = local_cache[part][lo_addr]
    if med_addr > 0:
        m_byt = local_cache[part][med_addr]
        med_val = ((m_byt >> med_bit_pos) & 1) * 128
        h_val = local_cache[part][hi_addr] * 256
    else:
        med_val = 0
        h_val = 0
...
def set_setting(control_item, value):
    '''call dcx by string like this, ie: set_setting('Eq1.freq1', int(value)) '''
...
def set_settings(*control_val_pairs):
    '''call dcx by string like this, ie: set_setting(['Eq1.freq1', 34], ['Eq1.freq01', int(value)]) '''
    if comlink_write_enabled:
...
if __name__ == "__main__":
    ...
    def set_setting(control_item, value):
        try:
            _obj_name_split = control_item.split('_')
            _objstr = _obj_name_split[0]
            _namestr = _obj_name_split[1]
        except:
            return None
        _obj = getattr(Mapper, _objstr, None)
        print('_obj:', str(_obj))
        if _obj == None:
            return None
            _item = getattr(_obj, _namestr, None)
        if _item == None:
            return None
        command =  _item.cmd + _sevenbit(value)
        port.write(DCX_HEAD + DCX_RX_ENABLE + DCX_TAIL)
