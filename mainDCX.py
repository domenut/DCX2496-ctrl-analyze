...
from gui_levelsUnit import Ui_LevelsFrame
from extras import PlotsMixin, ControlLinksMixin, FilesMixin, SpectrumAnalyserMixin
from metersDCX import Meters
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow, Ui_EqFrame, Ui_LevelsFrame, ControlLinksMixin, PlotsMixin, FilesMixin, SpectrumAnalyserMixin):
    ...
    def initiate_gui(self): # FIXME
        ...
        self.init_plots()
        #dcx.comlink_write_enabled = False # DEBUG ONLY
    ...
    def WASmute_all(self, mute=None):
        if mute == None:
    ...
    def move_remote_gain_frame(self, channel):  # TODO (? needed ?)
        #return
        if 'chS' in channel:
            return
        elif 'ch' in channel:
    ...
    def tab_changed(self):
        sender = self.sender()
        self.write_ini('Tab', 'last', sender.currentWidget().objectName())
    ...
    def wire_spectrum_analyzer_widgets_to_targets(self):#TODO
        self.specrumPowerButton.toggled.connect(self.power_button_handler)
        self.spectrumNumAverages.valueChanged.connect(self.averages_handler)
        self.spectrumVShift.valueChanged.connect(self.vertical_shift_handler)
        self.spectrumFftLength.currentIndexChanged.connect(self.fft_length_box_handler)
        self.smoothButton.toggled.connect(self.smoothing_button_handler)
        self.spectrumFreezePlotButton.toggled.connect(self.freeze_plot_button_handler)
        self.smoothingFactorSpinBox.valueChanged.connect(self.smoothing_factor_handler)
    ...
    def wire_dcx_control_widgets_to_targets(self):
